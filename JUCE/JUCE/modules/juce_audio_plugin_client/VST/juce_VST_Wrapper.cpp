...
String getWusikX4_WConfig_Value(String xname)
{
	#if JUCE_WINDOWS
		File fConfig = File(File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFullPathName().replace(".dll", ".wconfig"));
	#else
		File fConfig = File(File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFullPathName().replace(".vst", ".wconfig"));
	#endif
	if (fConfig.exists())
	{
         ...
         processor (af)
    {
		...
		if (fConfig.existsAsFile())
		{
			...
			else
			{
				XmlElement* plugDetails = myXML->getChildByName("PLUGIN");
				//
				vstEffect.uniqueID = plugDetails->getIntAttribute("uid");
				vstEffect.numInputs = plugDetails->getIntAttribute("numInputs");
				vstEffect.numOutputs = plugDetails->getIntAttribute("numOutputs");
				vstEffect.numParams = plugDetails->getIntAttribute("numParameters");
				vstEffect.numPrograms = plugDetails->getIntAttribute("numPrograms");
				//
				StringArray segments = StringArray::fromTokens(plugDetails->getStringAttribute("version"), ",.", "");
				segments.trim();
				segments.removeEmptyStrings();
				int value = (segments[0].getIntValue() << 16) + (segments[1].getIntValue() << 8) + segments[2].getIntValue();
				vstEffect.version = value;
				//
				myXML = nullptr;
				processor->setPlayConfigDetails(vstEffect.numInputs, vstEffect.numOutputs, 44100.0, 1024);
			}
		}
...
private:
    ...
    {
		String (File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFileNameWithoutExtension().replace(" 32", "").replace(" 64", "").copyToUTF8 ((char*) args.ptr, 64 + 1));
        return 1;
    }

	pointer_sized_int handleGetManufacturerName(VstOpCodeArguments args)
	{
		String theName = getWusikX4_WConfig_Value("manufacturer");
		if (theName.isNotEmpty())
		{
			theName.copyToUTF8((char*)args.ptr, 64 + 1);
			return 1;
		}
        //
    ...
    pointer_sized_int handleGetManufacturerVersion (VstOpCodeArguments)
    {
		...
		if (theName.isNotEmpty())
		{
			StringArray segments = StringArray::fromTokens(theName, ",.", "");
			segments.trim();
			segments.removeEmptyStrings();
			//
