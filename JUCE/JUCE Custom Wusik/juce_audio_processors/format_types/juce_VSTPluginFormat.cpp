                               ...
                               private AsyncUpdater
{
    ...
    void fillInPluginDescription (PluginDescription& desc) const override
    {
		...
		desc.midiIN = wantsMidiMessages;
		desc.midiOUT = pluginCanDo("sendVstMidiEvent") > 0;

		if (vstEffect != nullptr)
		{
			desc.numParameters = vstEffect->numParams;
			desc.numPrograms = vstEffect->numPrograms;
			//
			#if WUSIK_X42
				for (int xI = 0; xI < desc.numPrograms; xI++)
				{
					desc.programNames.add(getTextForOpcode(xI, Vst2::effGetProgramNameIndexed));
				}
				//
				for (int xI = 0; xI < desc.numParameters; xI++)
				{
					desc.parameterNames.add(getTextForOpcode(xI, Vst2::effGetParamName));
				}
			#endif
		}
    }
...
public:
    ...
    void mouseDown (const MouseEvent& e) override
    {
		...
		//AlertWindow::showMessageBox(AlertWindow::NoIcon, "", "Alert!");
