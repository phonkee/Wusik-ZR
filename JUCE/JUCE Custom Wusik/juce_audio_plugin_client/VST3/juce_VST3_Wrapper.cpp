...
juce::String getWusikX4_WConfig_Value(juce::String xname)
{
	juce::File fConfig = juce::File(juce::File::getSpecialLocation(juce::File::SpecialLocationType::currentApplicationFile).getFullPathName().replace(".vst3", ".wconfig"));
	if (fConfig.exists())
	{
		juce::ScopedPointer<juce::XmlElement> myXML = nullptr;
		juce::XmlDocument xDoc(fConfig);
		myXML = xDoc.getDocumentElement();
		if (myXML == nullptr) juce::AlertWindow::showMessageBox(juce::AlertWindow::NoIcon, "Fatal Error Reading Wusik X42 Configuration File", fConfig.getFileName());
		else
		{
			juce::XmlElement* plugDetails = myXML->getChildByName("PLUGIN");
			return plugDetails->getStringAttribute(xname);
		}
	}
	//
	return juce::String();
...
{
	String sPluginName = JucePlugin_Name;
	int xID = JucePlugin_VSTUniqueID;

	// !!!! WUSIK HACK !!!!! //

	String theName = getWusikX4_WConfig_Value("uid"); 
	if (theName.isNotEmpty()) xID = theName.getIntValue();
    ...
    char uidStr[9] = { 0 };
    sprintf (uidStr, "%08X", xID);
    strcat (uidString, uidStr);

    char nameidStr[3] = { 0 };
    const size_t len = strlen (sPluginName.toRawUTF8());

    for (size_t i = 0; i <= 8; ++i)
    {
        juce::uint8 c = i < len ? static_cast<juce::uint8> (sPluginName.toRawUTF8()[i]) : 0;
...
JUCE_EXPORTED_FUNCTION IPluginFactory* PLUGIN_API GetPluginFactory()
{
    ...
    if (globalFactory == nullptr)
    {
		...
		juce::String sVersion = JucePlugin_VersionString;
		bool isFX = juce::File::getSpecialLocation(juce::File::SpecialLocationType::currentApplicationFile).getFileNameWithoutExtension().containsIgnoreCase(" fx ");
		juce::String sManufacturer = JucePlugin_Manufacturer;
		juce::String sFileName = File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFileNameWithoutExtension();
		//
		File fConfig = File(File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFullPathName().replace(".vst3", ".wconfig"));
		StringArray xConfigurations;
		if (fConfig.existsAsFile())
		{
			ScopedPointer<XmlElement> myXML = nullptr;
			XmlDocument xDoc(fConfig);
			myXML = xDoc.getDocumentElement();
			if (myXML == nullptr) AlertWindow::showMessageBox(AlertWindow::NoIcon, "Fatal Error Reading Wusik X42 Configuration File", fConfig.getFileName());
			else
			{
				XmlElement* plugDetails = myXML->getChildByName("PLUGIN");
				//
				isFX = (!(plugDetails->getStringAttribute("isInstrument").compareIgnoreCase("1") == 0));
				sVersion = plugDetails->getStringAttribute("version");
				sManufacturer = plugDetails->getStringAttribute("manufacturer");
				sFileName = plugDetails->getStringAttribute("name");
				//
				myXML = nullptr;
			}
		}
