...
public:
          ...
          mapper (*juceFilter)
    {
		...
		File fConfig = File(File::getSpecialLocation(File::SpecialLocationType::currentApplicationFile).getFullPathName().replace(".component", ".wconfig"));
		...
		if (fConfig.existsAsFile())
		{
			...
			else
			{
				...
				//
				totalInChannels = plugDetails->getIntAttribute("numInputs");
				totalOutChannels = plugDetails->getIntAttribute("numOutputs");
				//
				myXML = nullptr;
			}
		}
    ...
    void GetUIComponentDescs (ComponentDescription* inDescArray) override
    {
		...
		if (fConfig.existsAsFile())
		{
			...
			else
			{
				XmlElement* plugDetails = myXML->getChildByName("PLUGIN");
				//
				inDescArray[0].componentSubType = plugDetails->getIntAttribute("uid");
				inDescArray[0].componentManufacturer = plugDetails->getIntAttribute("manufacturer");
				//
				myXML = nullptr;
			}
		}
