...
//==============================================================================
class WAudioDeviceSelectorComponent::MidiInputSelectorComponentListBox  : public ListBox,
...
public:
    ...
    {
        if (WAudioDeviceSelectorComponent* parent = findParentComponentOfClass<WAudioDeviceSelectorComponent>())
        {
...
//==============================================================================
WAudioDeviceSelectorComponent::WAudioDeviceSelectorComponent (AudioDeviceManager& dm,
      ...
      hideAdvancedOptionsWithButton (hideAdvancedOptionsWithButton_)
{
	...
	viewPortComponent = new Component();
	viewPort = new Viewport("Audio Device");
    ...
    if (types.size() > 1)
    {
		...
		viewPortComponent->addAndMakeVisible(deviceTypeDropDown);
    ...
    {
		viewPortComponent->addAndMakeVisible(midiInputsList
        ...
        {
			viewPortComponent->addAndMakeVisible(bluetoothButton = new TextButton(TRANS("Bluetooth MIDI"),
    ...
    {
		viewPortComponent->addAndMakeVisible(midiOutputSelector = new ComboBox(String()));
...
WAudioDeviceSelectorComponent::~WAudioDeviceSelectorComponent()
{
    deviceManager.removeChangeListener (this);
}

void WAudioDeviceSelectorComponent::setItemHeight (int newItemHeight)
{
    itemHeight = newItemHeight;
    resized();
}

void WAudioDeviceSelectorComponent::resized()
{
    Rectangle<int> r (proportionOfWidth (0.35f), 15, proportionOfWidth (0.6f), 3000);
    const int space = itemHeight / 4;

	viewPort->setBounds(0, 0, getWidth(), getHeight() - 80);
	viewPortComponent->setBounds(0, 0, getWidth(), 1200);
	...
	okButton->setBounds(120, getHeight() - 52, 120, 40);
	cancelButton->setBounds(280, getHeight() - 52, 120, 40);
}

void WAudioDeviceSelectorComponent::timerCallback()
{
    // TODO
	...
	prevPortPos = viewPort->getViewPositionY();
    updateAllControls();
	viewPort->setViewPosition(0, prevPortPos);
}

void WAudioDeviceSelectorComponent::comboBoxChanged (ComboBox* comboBoxThatHasChanged)
{
...
void WAudioDeviceSelectorComponent::changeListenerCallback (ChangeBroadcaster*)
{
    updateAllControls();
}

void WAudioDeviceSelectorComponent::updateAllControls()
{
    if (deviceTypeDropDown != nullptr)
         ...
         || audioDeviceSettingsCompType != deviceManager.getCurrentAudioDeviceType())
    {
                                                            ...
                                                            ? 0 : deviceTypeDropDown->getSelectedId() - 1])
        {
			...
			viewPortComponent->addAndMakeVisible(sp);
...
void WAudioDeviceSelectorComponent::buttonClicked (Button* btn)
{
    if (bluetoothButton == btn)
        BluetoothMidiDevicePairingDialogue::open();
	else if (okButton == btn)
	{
		parentWindow->exitModalState(1);
		((StandalonePluginHolder*) owner)->saveAudioDeviceState();
	}
	else if (cancelButton == btn)
	{
		parentWindow->exitModalState(0);
		((StandalonePluginHolder*) owner)->reloadAudioDeviceState("", nullptr);
	}
}

ListBox* WAudioDeviceSelectorComponent::getMidiInputSelectorListBox() const noexcept
{
